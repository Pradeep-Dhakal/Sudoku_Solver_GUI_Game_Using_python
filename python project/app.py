...
def solve_sudoku():
    global number_list
    if SudokoSolver.check_correct(number_list) != 0:
        SudokoSolver.solve(number_list)
    else:
        tkinter.messagebox.showerror('HATA',"Yazılan sudoku'da yanlış var!!!")
    display_numbers()

button_solve = tk.Button(win, text = 'solve', command = solve_sudoku)
button_clear_all = tk.Button(win, text = 'clear board', command = clear_list_all)
